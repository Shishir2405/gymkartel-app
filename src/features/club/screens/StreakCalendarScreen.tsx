...
/**
 * Streak calendar. A month grid marks recorded check-in days, today carries the
 * only orange (the active flame), the streak rule is stated plainly in a card,
 * and the current streak count is shown in Barlow.
 */
export function StreakCalendarScreen(_props: MemberScreenProps<"StreakCalendar">) {
  const cells = buildMonthCells(CALENDAR_YEAR, CALENDAR_MONTH);
  const checkedIn = new Set(CHECK_IN_DAYS);
  const monthName = MONTH_NAMES[CALENDAR_MONTH] ?? "";
    ...
    <Screen scroll testID="streak-calendar">
      ...
      <Card padded style={styles.streakCard}>
        ...
        <View style={styles.streakRow}>
          <Text preset="displayLarge">{MOCK_STREAK_DAYS}</Text>
